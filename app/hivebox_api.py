...
def get_senseBox_data(box_id):
    ...
    if response.status_code == 200:
        ...
        logging.info(f"Failed to fetch data for box ID: {box_id}. Status code: {response.status_code}")
        return None
    ...
    if senseBox_data:
        ...
        for sensor_data in all_sensor_data:
            if sensor_data.get("title") == "Temperatur":
                logging.info(f"Successfully fetched temperature for box ID: {box_id} with temperature: {sensor_data.get('lastMeasurement').get('value')}")
